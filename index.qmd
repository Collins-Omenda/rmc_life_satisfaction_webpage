...
table_data <- cantril %>% select(!Code) %>%
  ...
  pivot_wider(names_from = Year, 
    ...
    `Relative change` = percent(`Relative change`, accuracy = 0.1)
  )
...
countries <- c("Kenya", "Tanzania", "Uganda", "South Africa", "Nigeria", "Ghana")
cantril %>% select(`Cantril ladder score`)
f_data <- cantril %>%
  filter(Country %in% countries)

s_plot <- ggplot(f_data, aes(x = Year, y = `Cantril ladder score` , color = Country)) +
  geom_line(size = 1) +
...
## 📈 Top 10 and Bottom 10 Countries (2023)
...
top_bottom_data <- cantril %>%
  filter(Year == 2023) %>%
  select(Country, `Cantril ladder score`) %>%
  arrange(desc(`Cantril ladder score`))

# 2. Get top 10 and bottom 10
top_10 <- top_bottom_data %>% slice_head(n = 10)
bottom_10 <- top_bottom_data %>% slice_tail(n = 10)

# 3. Combine and add a 'Group' label
combined_data <- bind_rows(
  top_10 %>% mutate(Group = "Top 10"),
  bottom_10 %>% mutate(Group = "Bottom 10")
)

# 4. Plot the data
ggplot(combined_data, aes(x = reorder(Country, `Cantril ladder score`), y = `Cantril ladder score`, fill = Group)) +
  geom_col() +
  coord_flip() +
  labs(
